...
function r9ls_theme_product_map(){return array(
 'daily'=>array('morning-brief','todays-forecast','seven-day-forecast','headlines','weekly-hazards','watching'),
 'hazards'=>array('severe-weather-risk','threat-breakdown','storm-timing','tornado-risk','damaging-wind','large-hail','lightning-risk','flood-heavy-rain'),
 'agriculture'=>array('agriculture','fieldwork','spraying','harvest','livestock'),
 'travel-outdoor'=>array('travel','rural-travel','outdoor','outdoor-planner','schools','construction'),
 'special'=>array('forecast-confidence','decision-support-brief','watching','weekly-hazards'),
 'temperature-outlook'=>array('todays-forecast','seven-day-forecast','forecast-confidence'),
 'precipitation-outlook'=>array('forecast-rainfall','flood-heavy-rain','todays-forecast','decision-support-brief'),
 'severe-weather'=>array('severe-weather-risk','threat-breakdown','storm-timing','tornado-risk','damaging-wind','large-hail','lightning-risk','flood-heavy-rain'),
 'decision-dashboard'=>array('decision-support-brief','travel','agriculture','outdoor','schools','construction','forecast-confidence'),
 'rural-planning'=>array('rural-travel','agriculture','fieldwork','spraying','harvest','livestock','outdoor-planner','forecast-rainfall')
);}
...
function r9ls_theme_card($id){
 $p=r9ls_theme_product($id);
 if(!$p)return '<article class="r9-panel r9ls-product-card is-unavailable"><h3>'.esc_html(ucwords(str_replace('-',' ',$id))).'</h3><p>'.esc_html(r9ls_theme_fallback_language()).'</p></article>';
 $graphic='';
 if(!empty($p['graphic_url'])){
   $graphic='<a class="r9ls-graphic-link" href="'.esc_url($p['graphic_url']).'" target="_blank" rel="noopener" aria-label="Open full '.esc_attr($p['title']).' graphic"><img class="r9-image" src="'.esc_url($p['graphic_url']).'" alt="'.esc_attr($p['title']).' graphic"><span class="r9ls-open-graphic">Open full graphic →</span></a>';
 } else {$graphic=r9_media_placeholder(($p['title']??'Forecast').' Graphic');}
 return '<article class="r9-panel r9ls-product-card" data-product="'.esc_attr($id).'">'
 .'<div class="r9-panel-head"><div><span class="r9-eyebrow">REGION 9 APPROVED PRODUCT</span><h3>'.esc_html($p['title']??$id).'</h3></div>'.r9ls_theme_risk_badge($p['risk']??array()).'</div>'
 .$graphic
 .'<div class="r9ls-discussion-box"><h4>Forecast Discussion</h4>'.wp_kses_post(wpautop($p['discussion']??'')).'</div>'
 .'<p class="r9ls-summary">'.esc_html($p['summary']??'').'</p>'.r9ls_theme_confidence($p).r9ls_theme_time($p).r9ls_theme_counties($p)
 .'<p class="r9ls-forecaster"><strong>Forecaster:</strong> '.esc_html($p['forecaster']??'NEAL').'</p>'.r9ls_theme_updated($p).'</article>';
}
