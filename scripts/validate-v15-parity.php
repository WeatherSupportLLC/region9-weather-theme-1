...
foreach (array('daily','hazards','temperature-outlook','precipitation-outlook','travel-outdoor','agriculture','special','severe-weather') as $slug) { ok('catalog or product map keeps forecast page ' . $slug, strpos($functions . body('inc/live-studio-integration.php'), "'$slug'") !== false); }
...
ok('public product shortcode remains', strpos(body('inc/live-studio-integration.php'), "add_shortcode('r9ls_public_product'") !== false);
...
$report = array('passes'=>$passes,'generated_at'=>date('c'),'baseline'=>'v15/v15.1 artifacts unavailable; current 17.1 inventory guarded','purpose'=>'Fail if required pages, menus, sidebars, placeholders, admin pages, REST routes, theme relationship, or inventory docs disappear.');
