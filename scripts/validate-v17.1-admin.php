...
$build=$root.'/build';if(!is_dir($build)){mkdir($build,0777,true);}file_put_contents($build.'/v17.1-admin-report.json',json_encode(array('version'=>'17.1.0','assertions'=>$passes,'failures'=>0,'status'=>'PASS','generated_at'=>date('c')),JSON_PRETTY_PRINT));
