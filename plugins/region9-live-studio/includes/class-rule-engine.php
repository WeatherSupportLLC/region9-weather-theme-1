...
class R9LS_Rule_Engine {
    ...
    public function __construct($gis, $audit) { $this->gis = $gis; $this->audit = $audit; }
    ...
    public function evaluate_all($sources) {
        ...
        $out = array();
        foreach ($this->products as $product) { $out[$product] = $this->evaluate_product($product, $sources, $geo); }
        return $out;
    }
    ...
    public function evaluate_product($product, $sources, $geo) {
        ...
        foreach (array('spc','ero','alerts') as $source_key) {
            ...
            if ($result['status'] !== 'ok') { $confidence -= 20; $trace[] = array('source'=>$source_key,'effect'=>'confidence -20','reason'=>'source unavailable or malformed'); continue; }
            $weight = $rules['weights'][$source_key] ?? 0; $source_score = $result['highest_risk'] * $weight; $score += $source_score;
            foreach ($result['county_risks'] as $county => $risk) { $county_scores[$county] = min(100, $county_scores[$county] + ($risk * $weight)); }
        ...
        $qpf = $geo['qpf'];
        if (($qpf['status'] ?? 'failure') !== 'healthy') { $confidence -= 10; $trace[] = array('source'=>'qpf','effect'=>'confidence -10','reason'=>'national QPF unavailable'); }
        else {
            $qpf_weight = $rules['weights']['qpf'] ?? 4;
            foreach ((array)($qpf['county_precipitation'] ?? array()) as $county => $amount) { if ($amount !== null) $county_scores[$county] = min(100, $county_scores[$county] + ((float)$amount * $qpf_weight)); }
            $max_qpf = empty($qpf['county_precipitation']) ? 0 : max(array_map('floatval', array_filter($qpf['county_precipitation'], 'is_numeric')) ?: array(0));
            $added = min(20, $max_qpf * $qpf_weight); $score += $added; $trace[] = array('source'=>'qpf','max_inches'=>$max_qpf,'weight'=>$qpf_weight,'score_added'=>$added);
        }
        ...
        return array(
            'score'=>$score,'rating'=>$this->rating($product,$score),'confidence'=>$confidence,
            'primary_drivers'=>array_values(array_unique($primary)),'secondary_drivers'=>array_values(array_unique($secondary)),
            'affected_counties'=>$this->affected_counties($county_scores),'timing'=>$this->timing($sources),
            'controlled_summary'=>$this->summary($product,$score,$confidence),'rule_trace'=>$trace,'county_scores'=>$county_scores,
    ...
    private function rating($product,$score) { if ($product==='Travel') { if($score<25)return'Good'; if($score<50)return'Caution'; if($score<75)return'Difficult'; return'Dangerous'; } return $this->region9_risk($score)['label']; }
    private function affected_counties($scores) { return array_keys(array_filter($scores, function($score){ return $score > 0; })); }
    private function timing($sources) { $times=array(); foreach($sources as $source) foreach(($source['hazards']??array()) as $hazard) if(!empty($hazard['timing']))$times[]=sanitize_text_field($hazard['timing']); return $times?implode('; ',array_unique($times)):'No hazardous timing identified.'; }
    private function summary($product,$score,$confidence) { return sprintf('%s operational score %d with %d%% confidence. Publication is controlled by the Region 9 automation and review policy.', $product,$score,$confidence); }
    private function clamp($value) { return max(0,min(100,(int)round($value))); }
}
