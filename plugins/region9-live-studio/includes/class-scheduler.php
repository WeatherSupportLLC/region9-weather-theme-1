...
class R9LS_Scheduler {
    ...
    const GRAPHICS_INTERVAL = 21600; // 6 hours
    ...
    public function cron_schedules($schedules) {
        ...
        $schedules['r9ls_active_weather']=array('interval'=>$minutes*MINUTE_IN_SECONDS,'display'=>'Region 9 five-minute weather validation');
    ...
    public function active_interval_minutes(){ $settings=get_option(self::SETTINGS,array()); $minutes=isset($settings['active_interval_minutes'])?absint($settings['active_interval_minutes']):5; return max(5,$minutes); }

    public function schedule_event() {
        if(wp_next_scheduled(self::HOOK)){ $this->set_health('scheduled','Existing scheduler event retained.'); return false; }
        $ok=wp_schedule_event(time()+60,'r9ls_active_weather',self::HOOK);
        if(!$ok){$this->audit->write('error','Scheduler failed to schedule validation event.');$this->set_health('failure','Failed to schedule validation event.');return false;}
        $this->set_health('scheduled','Five-minute validation event scheduled.'); return true;
    }
    ...
    public function validate($mode='manual') {
        ...
        try {
            ...
            if($this->products instanceof R9LS_Product_Generator){
                ...
                if($render_graphics){
                    update_option(self::LAST_GRAPHICS,time(),false);
                    $this->audit->write('info','Region 9 graphics/discussion cycle completed.',array('reason'=>$mode==='manual'?'manual':($severe_changed?'severe_material_change':'six_hour_cycle'),'automatic_publish'=>$auto_graphics));
                }
            }

            $duration=round(microtime(true)-$started,3);
            $last=array(
                'time'=>current_time('mysql'),'mode'=>sanitize_key($mode),'duration'=>$duration,'changes'=>count($changes),
                'graphics_rendered'=>(bool)$render_graphics,'graphics_reason'=>$render_graphics?($severe_changed?'severe_material_change':($mode==='manual'?'manual':'six_hour_cycle')):'not_due',
                'severe_weather_active'=>(bool)$severe,'next_graphics_due'=>gmdate('c',(int)get_option(self::LAST_GRAPHICS,time())+self::GRAPHICS_INTERVAL)
            );
            ...
            $this->audit->write('error','Validation failed.',array('error'=>$e->getMessage())); $this->set_health('failure','Validation failed: '.$e->getMessage()); return array('status'=>'error','message'=>$e->getMessage());
