...
class R9LS_REST_API {
    ...
    public function read($kind, $req = null) {
        if ($kind === 'metadata') { return array('component'=>'Region 9 Live Studio','version'=>R9LS_VERSION,'schema_version'=>R9LS_Product_Generator::VERSION,'namespace'=>self::NS,'publication_state'=>'approved_published_only'); }
    ...
    private function history() { $h = get_option(R9LS_Product_Generator::PUBLICATION_HISTORY, array()); return array_map(function($i){ return array('event_type'=>$i['event_type'] ?? 'publish','publication_reference'=>$i['publication_reference'] ?? '','product_id'=>$i['product_id'] ?? '','product_version'=>(int)($i['product_version'] ?? 0),'published_timestamp'=>$i['timestamp'] ?? '','actor'=>$i['actor'] ?? '','approval_reference'=>$i['approval_reference'] ?? '','previous_version'=>(int)($i['previous_version'] ?? 0),'rollback_reference'=>$i['rollback_reference'] ?? '','content_hash'=>$i['content_hash'] ?? ''); }, (array)$h); }
...
function r9ls_get_published_products() { $all=get_option(R9LS_Product_Generator::PRODUCTS,array());$out=array();foreach((array)$all as $id=>$product){if(($product['approval_state']??'')==='approved'&&($product['publication_state']??'')==='published'){$out[$id]=$product;}}return $out; }
...
function r9ls_get_widget_payload($product_id = 'todays-forecast') { $p = r9ls_get_published_product($product_id); return $p && ($p['approval_state'] ?? '') === 'approved' && ($p['publication_state'] ?? '') === 'published' ? array('product_id'=>$p['product_id'],'title'=>$p['title'],'summary'=>$p['summary'],'risk'=>$p['risk'],'updated_at'=>$p['updated_at']) : array(); }
