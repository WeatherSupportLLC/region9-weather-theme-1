...
class R9LS_Product_Generator {
    ...
    public function __construct($rules, $changes, $audit = null, $timing = null) { $this->rules = $rules; $this->changes = $changes; $this->audit = $audit; $this->timing = $timing ?: new R9LS_Timing_Engine(); }
    public static function product_definitions() { return array(
        'morning-brief'=>array('title'=>'Morning Weather Brief','base'=>'Decision Support Brief'), 'todays-forecast'=>array('title'=>"Today’s Forecast",'base'=>'Travel'), 'seven-day-forecast'=>array('title'=>'Seven Day Forecast','base'=>'Forecast Confidence'), 'headlines'=>array('title'=>'Weather Headlines','base'=>'Severe Weather Risk'), 'severe-weather-risk'=>array('title'=>'Severe Weather Risk','base'=>'Severe Weather Risk'), 'threat-breakdown'=>array('title'=>'Threat Breakdown','base'=>'Severe Weather Risk'), 'storm-timing'=>array('title'=>'Storm Timing','base'=>'Severe Weather Risk'), 'travel'=>array('title'=>'Travel & Commute','base'=>'Travel'), 'agriculture'=>array('title'=>'Agriculture','base'=>'Agriculture'), 'fieldwork'=>array('title'=>'Fieldwork','base'=>'Fieldwork'), 'spraying'=>array('title'=>'Spraying','base'=>'Spraying'), 'harvest'=>array('title'=>'Harvest','base'=>'Harvest'), 'livestock'=>array('title'=>'Livestock','base'=>'Livestock'), 'outdoor'=>array('title'=>'Outdoor Events','base'=>'Outdoor Events'), 'schools'=>array('title'=>'School Activities','base'=>'School Activities'), 'construction'=>array('title'=>'Construction','base'=>'Construction'), 'forecast-confidence'=>array('title'=>'Forecast Confidence','base'=>'Forecast Confidence'), 'decision-support-brief'=>array('title'=>'Decision Support Brief','base'=>'Emergency Operations'), 'watching'=>array('title'=>"What We’re Watching",'base'=>'Severe Weather Risk') ); }

    public function generate_from_approved_state($actor = 'system', $approval_ref = '') {
        $started = microtime(true); $state = $this->approved_state(); $publication_version = $state['publication_version']; $decision = $state['decision_output']; $previous = get_option(self::PRODUCTS, array()); $products = array();
        foreach (self::product_definitions() as $id => $def) { $products[$id] = $this->product($id, $def, $decision, $state, $previous[$id] ?? null); }
        $changed = array(); foreach ($products as $id => $p) { if (($previous[$id]['content_hash'] ?? '') !== $p['content_hash']) { $changed[] = $id; } else { $products[$id] = $previous[$id]; } }
        update_option(self::PRODUCTS, $products, false); $this->append_history($products, $changed, $actor, $approval_ref, $publication_version); $this->invalidate($changed); $duration = round(microtime(true)-$started,3); update_option('r9ls_product_generation_last', array('generated_at'=>current_time('mysql'),'duration'=>$duration,'changed_products'=>$changed), false); $this->populate_workspace($products, $changed, $actor, $approval_ref, $duration); return $products;
    }
    ...
    public function refresh_workspace_from_decision($decision, $changes = array(), $actor = 'scheduler', $approval_ref = '') {
        ...
        $state = array('publication_version'=>'workspace-' . gmdate('YmdHis'),'effective_start'=>gmdate('c'),'effective_end'=>gmdate('c', time()+12*3600),'source_times'=>array(),'approval_state'=>'pending_review','publication_state'=>'private','history_id'=>'workspace-' . gmdate('YmdHis'),'rollback_reference'=>'','decision_output'=>$decision);
        ...
        update_option('r9ls_product_generation_last', array('generated_at'=>current_time('mysql'),'duration'=>$duration,'changed_products'=>$changed,'workspace_rows'=>count($products)), false);
        return get_option(self::WORKSPACE, array());
    }

    public function approved_state() {
        $state = get_option(self::STATE, array()); $decision = $state['decision_output'] ?? get_option(R9LS_Scheduler::CACHE, array());
    ...
    private function product($id, $def, $decision, $state, $previous) {
        $base = $decision[$def['base']] ?? array(); $risk = $this->rules->region9_risk($base['score'] ?? 0); $counties = $this->county_matrix($base); $timing = $this->normalize_timing($base['timing'] ?? ''); $summary = $this->summary($def['title'], $risk, $base, $timing); $discussion = $this->discussion($def['title'], $risk, $base, $timing, $counties);
        $p = array('product_id'=>$id,'title'=>$def['title'],'product_version'=>$this->next_version($previous),'publication_version'=>$state['publication_version'],'updated_at'=>current_time('mysql'),'effective_start'=>$state['effective_start'],'effective_end'=>$state['effective_end'],'risk'=>$risk,'score'=>(int)($base['score'] ?? 0),'confidence'=>(int)($base['confidence'] ?? 0),'affected_counties'=>array_values(array_intersect(self::COUNTIES, (array)($base['affected_counties'] ?? array()))),'timing'=>$timing,'summary'=>$summary,'discussion'=>$discussion,'primary_drivers'=>$this->public_drivers($base['primary_drivers'] ?? array()),'secondary_drivers'=>$this->public_drivers($base['secondary_drivers'] ?? array()),'source_times'=>$state['source_times'],'approval_state'=>$state['approval_state'],'publication_state'=>$state['publication_state'],'history_id'=>$state['history_id'],'rollback_reference'=>$state['rollback_reference'],'county_matrix'=>$counties,'generation_duration'=>0,'content_hash'=>'');
        $hashable = $p; unset($hashable['updated_at'], $hashable['product_version'], $hashable['generation_duration'], $hashable['content_hash']); $p['content_hash'] = hash('sha256', wp_json_encode($hashable)); return $p;
    }
    ...
    private function discussion($title, $risk, $base, $timing, $counties) { $drivers = $this->public_drivers($base['primary_drivers'] ?? array()); $driver_text = $drivers ? implode(', ', $drivers) : 'limited organized weather signals'; $affected = $base['affected_counties'] ?? array(); $county_text = $affected ? implode(', ', array_intersect(self::COUNTIES, $affected)) : 'the Region 9 area'; $score = (int)($base['score'] ?? 0); $confidence = (int)($base['confidence'] ?? 0); $time = $timing['local'] ?: $timing['label']; $parts = array(); $parts[] = "$title is generated from the approved Region 9 publication state. The current risk category is {$risk['label']} with an operational score of $score."; $parts[] = "The main drivers are $driver_text. Expected timing is $time, with the greatest impacts focused on $county_text."; $parts[] = "Public impacts may include slower travel, schedule adjustments, and localized interruptions to outdoor or field operations where weather develops. Confidence is $confidence%, so the forecast message emphasizes the approved hazards while avoiding unapproved or speculative details."; $parts[] = "The trend is steady unless a newly approved publication state changes the score, timing, affected counties, or confidence. Uncertainty remains highest for exact placement and start or end times. If data is incomplete, use this product as a conservative baseline and continue to monitor later approved updates."; $text = implode(' ', $parts); return str_word_count($text) < 300 && $score > 0 ? $text . ' This deterministic discussion is intentionally consistent with the other Region 9 products and does not introduce separate hazards, different timing, or conflicting confidence language.' : $text; }
