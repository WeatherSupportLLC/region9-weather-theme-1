...
class R9LS_Product_Generator {
    ...
    /** Backward-compatible explicit approved-state publication used by pre-17.1 integrations. */
    ...
    public static function migrate_17_1($audit = null) {
        ...
        foreach (array(self::PRODUCTS, 'r9ls_pending_changes', 'r9ls_editor_overrides', 'r9ls_live_controls', 'r9ls_settings', 'r9ls_source_health', 'r9ls_canonical_alert_state') as $option) {
            $value = get_option($option, array());
            if (!is_array($value)) { update_option($option, array(), false); }
        }
        $marker = array('version'=>self::VERSION,'upgraded_from'=>'17.0.0-rc.1','status'=>'complete','completed_at'=>current_time('mysql'),'preserved_published_products'=>count((array)get_option(self::PRODUCTS, array())));
